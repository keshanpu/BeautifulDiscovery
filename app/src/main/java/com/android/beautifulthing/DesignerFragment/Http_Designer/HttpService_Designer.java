...
public interface HttpService_Designer {
    ...
    Observable<DesignerBean> queryDesigner(@Query("page")int page, @Query("page_size")int size);

    @GET("{basepath}/{url_path}")
    Observable<DesignerDetilBean> queryDesigner2(@Path("basepath")String basepath, @Path("url_path") String url_path);
}
