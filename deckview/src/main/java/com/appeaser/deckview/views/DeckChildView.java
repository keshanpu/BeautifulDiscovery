...
public class DeckChildView<T> extends FrameLayout implements
    ...
    public void setMyCircleImage(Bitmap mCircleImage) {
        this.mCircleImage.setImageBitmap(mCircleImage);
    }
    ...
    public void setMyData(Bitmap url_image,String iconTitle, String title, String subTitle){
        this.mImageView.setImageBitmap(url_image);
        this.iconTitle.setText(iconTitle);
...
//        mThumbnailView = (DeckChildViewThumbnail) findViewById(R.id.task_view_thumbnail);
        ...
        mImageView = (ImageView) findViewById(R.id.task_view_url_image);
        mCircleImage = (ImageView) findViewById(R.id.task_view_mycircleimage);
//        mThumbnailView.updateClipToTaskBar(mHeaderView);

        iconTitle = (TextView) findViewById(R.id.tash_view_icon_title);
        title = (TextView) findViewById(R.id.tash_view_title);
        subTitle = (TextView) findViewById(R.id.tash_view_subtitle);
...
//    }
    ...
    //改写上面的;
    public void onDataLoaded(T key, Bitmap url_image,
                             String iconTitle,
                             String title, String subTitle) {
        if (!isBound() || !mKey.equals(key))
            return;
        //调用自己写的接口;
        setMyData(url_image,iconTitle,title,subTitle);

        //其实mThumbnailView也可以改为我们setMy

        //aa
//        if (mThumbnailView != null){// && mHeaderView != null) {
//            // Bind each of the views to the new task data
//            mThumbnailView.rebindToTask(url_image);
//
////            mHeaderView.rebindToTask(headerIcon, headerTitle, headerBgColor);
////            // Rebind any listeners
////            mHeaderView.mApplicationIcon.setOnClickListener(this);
...
//        }
    ...
    public void onDataLoaded2(T key, Bitmap url_icon) {
        if (!isBound() || !mKey.equals(key))
            return;
        //调用自己写的接口;
        setMyCircleImage(url_icon);
